...
import { getAllProfilesFromDb, deleteProfileFromDb, generateSlug } from '@/lib/profile-store';
...
export default function HomePage() {
  ...
  useEffect(() => {
    async function load() {
      const data = await getAllProfilesFromDb();
      setProfiles(data);
  ...
  const handleShortenUrl = (profile: ProfileData) => {
    const shareableUrl = generateShareableUrl(profile, window.location.origin);
    window.open(`https://tinyurl.com/create.php?url=${encodeURIComponent(shareableUrl)}`, '_blank');
  };
        ...
        <div className="container mx-auto text-center max-w-3xl">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button
              ...
              size="lg"
              className="rounded-full px-8 py-6 text-lg font-semibold"
              onClick={() => window.open('https://tinyurl.com/', '_blank')}
            >
      ...
      <section className="py-12 px-4 border-t border-border/50">
        <div className="container mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {[
              { title: 'تخصيص كامل', description: 'ألوان، خطوط، خلفيات، وتأثيرات حركية', icon: '🎨' },
              { title: 'روابط التواصل', description: 'جميع منصات التواصل الاجتماعي في مكان واحد', icon: '🔗' },
              { title: 'وسائط متعددة', description: 'أضف صور، فيديوهات، وموسيقى لصفحتك', icon: '🎵' },
            ].map((feature, index) => (
                  ...
                  <Card key={profile.id} className="border-border/50 bg-card/50 overflow-hidden group">
                    ...
                    <CardContent className="p-4 pt-8">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="outline" size="icon" onClick={() => handleCopyLink(profile)} className="shrink-0">
                          {copiedId === profile.id ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                        </Button>
                        <Button variant="outline" size="icon" onClick={() => handleShowQRCode(profile)} className="shrink-0">
                          <QrCode className="w-4 h-4" />
                        </Button>
                        <Button variant="outline" size="icon" onClick={() => handleShortenUrl(profile)} className="shrink-0" title="تقصير الرابط">
                          <Link2 className="w-4 h-4" />
                        </Button>
                        <Button variant="outline" size="icon" className="shrink-0" onClick={() => handleOpenProfile(profile)}>
