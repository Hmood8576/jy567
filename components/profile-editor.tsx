            ...
            <CardContent className="px-4 pb-4 space-y-5">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-5 gap-2">
                  {(['color', 'gradient', 'image', 'animated', 'video'] as const).map((type) => (
                    <Button key={type} variant={profile.theme.backgroundType === type ? 'default' : 'outline'} onClick={() => updateTheme({ backgroundType: type })} className="w-full text-xs h-9" size="sm">
                      {type === 'color' ? 'لون' : type === 'gradient' ? 'تدرج' : type === 'image' ? 'صورة' : type === 'animated' ? 'متحركة' : 'فيديو'}
                    </Button>
                  ))}
                </div>
              </div>
              ...
              {profile.theme.backgroundType === 'video' && (
                <div className="space-y-3">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    ادعم روابط MP4 المباشرة. الفيديو سيعمل تلقائياً بدون صوت ويتكرر عند الانتهاء.
                  </p>
                </div>
              )}
